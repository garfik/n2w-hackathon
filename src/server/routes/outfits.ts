  ...
  "/api/outfits": {
    async GET(req) {
      ...
      const rows = await db
        ...
        .from(outfit)
        .where(eq(outfit.userId, result.userId))
    ...
    async POST(req) {
      ...
      let body: { garmentIds?: string[]; occasion?: string };
      try {
        body = (await req.json()) as { garmentIds?: string[]; occasion?: string };
      } catch {
      ...
      if (invalid.length > 0) {
        return Response.json(
          { error: "Some garments do not belong to you" },
          { status: 400 }
        );
      }
    ...
    return Response.json({
      ...
      outfit: {
        ...
        resultImageUrl: outfitRow.resultImageKey
          ? thumbnailUrl(outfitRow.resultImageKey)
          : null,
      },
    });
  },
